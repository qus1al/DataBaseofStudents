...
public:
	...
	void UpdateMasString(StudentNode* sn) {
		...
		stringMas.Add(sb.setParam("birthDateString", sn->birthDateString));
		int sem = 0;
		int num = 0;
	...
	double getAvgMarksSem(StudentNode* sn, int sem) {
		double sum = 0;
		double count = 0;
		for (int j = 0; j < 10; j++)
			if (sn->examsRecordsData[sem][j].isEmpty == false) {
				sum = sum + sn->examsRecordsData[sem][j].mark;
				count++;
			}
		int avr = 0;
		if (count > 0)
