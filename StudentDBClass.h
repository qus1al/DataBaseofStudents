...
public:
	...
	void loadDataFromFile() {
		string line;
		int count = 0;
		std::ifstream inFile(FileName); // îêğûâàåì ôàéë äëÿ ÷òåíèÿ
		if (inFile.is_open())
		{
			bool isRecord = false;
	...
	void saveDataToFile(string inFileName) {
		std::ofstream outFile;          // ïîòîê äëÿ çàïèñè
		ifstream iff(inFileName); //åñëè ôàéë åñòü óäàëÿåì
		if (iff.bad() == false)
		{
			iff.close();
		...
		outFile.open(inFileName, std::ios::app); // îòêğûâàåì ôàéë äëÿ çàïèñè
		if (outFile.is_open())
		{
	...
	void printAllSurName_Name_MName_sex_avrMarks() {
		...
		while (curr) {
			if (curr->sex == 1) {
				cout << curr->surName + " " + curr->name + " " + curr->middleName + " " + "Ì" + " " + std::to_string(curr->avrMarks) << endl;
				curr = curr->next;
			}
			else if (curr->sex == 0) {
				cout << curr->surName + " " + curr->name + " " + curr->middleName + " " + "Æ" + " " + std::to_string(curr->avrMarks) << endl;
				curr = curr->next;
			}
		}
		delete sb;
