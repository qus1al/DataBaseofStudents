...
public:
	...
	void saveDataToFile(string inFileName) {
		...
		outFile.open(inFileName, std::ios::app); // открываем файл для записи
		if (outFile.is_open())
		{
	...
	void printAllSurName_Name_MName_sex_avrMarks() {
		...
		while (curr) {
			if (curr->sex == 1) {
				cout << curr->surName + " " + curr->name + " " + curr->middleName + " " + "М" + " " + std::to_string(curr->avrMarks) << endl;
				curr = curr->next;
			}
			else if (curr->sex == 0) {
				cout << curr->surName + " " + curr->name + " " + curr->middleName + " " + "Ж" + " " + std::to_string(curr->avrMarks) << endl;
				curr = curr->next;
			}
		}
		delete sb;
